...
public class ServiceOfSection implements ISectionService {
    ...
    public ResponseEntity<ResponeObject> getSectionByCourseID(int id) {
        ...
        List<ResponeSectionDTO> responeSectionDTOS =  foundSection.stream().map(section -> fromSectionIntoResponeSectionDTO(section,id)).toList();
        if(responeSectionDTOS != null)
        {
            return ResponseEntity.status(HttpStatus.OK).body(
                    new ResponeObject("ok","Query product seccessfully",responeSectionDTOS)
            );
        }
        else
        {
